...
Moti の borrow は flow-sensitive かつ place-sensitive である。競合判定は canonical place に基づき、index/key からの自動非衝突推論には頼らない。public signature では必要な lifetime を明示できるが、`await` をまたいで lexical borrow や await-affine capability を保持してはならない。
