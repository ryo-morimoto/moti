...
Moti は trusted kernel を小さく保つため、strict positivity を要求し、codata は projection-only とし、guarded corecursion と小さい definitional equality を採用する。`extern fn`、`verified extern fn`、runtime 特例、実装都合の rewrite を definitional equality へ入れてはならない。
